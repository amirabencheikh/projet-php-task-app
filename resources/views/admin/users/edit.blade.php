...
<div class="bg-white shadow-lg px-4 py-6 rounded-md">
    <form method="post" action="{{route('admin.users.edit',['user'=>$user->id])}}">
        @csrf
        @foreach ($roles as $role)
        <div class="flex items-start mb-6">
            <div class="flex items-center h-5">
                <input id="role" type="checkbox" value="{{$role->id}}" name="roles[]" @if ($user->roles->contains('id', $role->id))
                checked
                @endif
                <div class="w-4 h-4 border-border-gray-300 rounded bg-gray-50 focus:ring-3 focus:ring-blue-300 dark:focus:ring-blue-600">
                </div>
